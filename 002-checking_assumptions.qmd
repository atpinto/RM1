...
As shown below, assumptions (1) - (3) can be defined in terms of the random part of the regression model - the residuals. The residuals are the observed minus the predicted Y's, i.e. $e_i = Y_i-\hat{Y}_i=Y_i-(b_0+b_1x_i)$.

::::: grid

::: g-col-7
Assumption (4) however is usually an assessment of the study design rather than assessed with a diagnostic analysis of the data. (**Note**: students have often forgotten about assumption 4 in the past, so make sure you don't forget this assumption when assessing the underlying assumptions of the linear regression model.)

These assumptions are explained using the HERS dataset (from last week) in the video.
:::
...
Note in this figure we plot the residuals against the *predicted* values (or *fitted* values) of the regression line instead of the $x$-values themselves. When there is just one predictor variable, as the regression line is linear this is equivalent to plotting against $x$. The advantage to plotting against the fitted value is that this extends naturally into multivariable models. From this *residual vs. fitted* plot, we can also see that the linearity assumption could be rewritten in terms of the residuals. E.g. the residuals have a mean of zero for all values of $x$.
...
This is usually an assessment of the study design rather than assessed with a diagnostic analysis of the data. This assumption can be violated, for example, if the study subjects have repeated measures taken (e.g. over time, or under different conditions (under the new study drug and then under the placebo)), or if there's some spatial relationship between the observations (e.g. individuals treated in the same hospital might have more similar outcomes compared to individuals treated in a different hospital).

More advanced statistical models are needed if this assumption is violated (outside the scope of RM1, but you might encounter these models in future units, such as "Longitudinal and Correlated Data" (LCD).)
...
Hence, by performing the log transformation, we can then use a more standard technique like linear regression to analyse the data.

The log transformation is a very natural and useful tool for many biological measures because it converts *multiplicative* relationships (such as those observed in processes such as cell division and successive dilution of assays) into *additive* relationships - which by the central limit theorem are more likely to be normally distributed. The regression equations that we've shown you so far have had *additive* relationships, e.g. $\text{E}(Y) =\beta_0 + \beta_1 x$ (note the $+$ sign for the additive relationship).

So on its original scale, an outcome variable might have a multiplicative relationship with a covariate, but once a log transformation is applied to the outcome variable, it then has an additive relationship with the covariate. For example, on a logarithmic scale the values 0.1, 1 and 10 equal $log(0.1) = -2.30$, $log(1) = 0$ and $log(10) = 2.30$, which are all equally spaced (with a difference of 2.3), whereas the original numbers (0.1, 1 and 10) all differ by a multiplicative factor of 10. Thus the log transformation created an additive relationship.

**Note** that in this course "Log" is used to designate the natural logarithm ($log_e$ or $ln$). But logarithms to any base will work equally well as this corresponds to rescaling the logged values.

In the example above the variable $Y$ is said to have a *log-normal* distribution as it is normally distributed after taking a log transformation. i.e. $\text{log}(Y) \sim N(\mu,\sigma^2)$. Performing a linear regression on an outcome variable that has been log-transformed, i.e. $log(Y)$ does require some care when interpreting the results on the original scale, as we outline below.

It is important to highlight some definitions of different types of "means", which you may have previously been unaware of (see the box below).

::: callout-note
-   The *arithmetic mean* is the "standard average" that you will be used to calculating, where you sum the data values and divide by the count, i.e., $\frac{1}{n} \sum_{i=1}^{n} Y_i$

-   The *geometric mean* (GM) is a different way of calculating an average in which you multiply the data values together and take the $n$-th root, i.e., $\sqrt[n]{Y_1 \cdot Y_2 \cdot \cdots \cdot Y_n}$
:::

::: {.callout-tip collapse="true"}
## Additional details (optional)

Assuming that the distribution of the log-transformed values ($logY_1, logY_2,..., logY_n$) is approximately symmetric, a natural measure of location on the log scale is the sample mean of ($logY_1, logY_2,..., logY_n$), i.e., $\frac{1}{n}\sum_{i=1}^{n} logY_i$. Back-transforming to the original scale then gives the (sample) geometric mean: $$
e^{\frac{1}{n} \sum_{i=1}^{n} \log Y_i} = \left( \prod_{i=1}^{n} Y_i \right)^{\frac{1}{n}}
$$
Also, if the distribution of values of ($logY_1, logY_2,..., logY_n$) is approximately symmetric, then the sample geometric mean is close in value to the sample median.
:::

For log-normally distributed variables, the **geometric mean** as defined by $GM(Y) = e^{\text{E}[log(Y)]} = e^{\mu}$ (i.e. by exponentiating the mean on the logarithmic scale), is a more suitable summary statistic than the arithmetic mean (that we normally use). We use the geometric mean for interpreting the results of a linear regression on its original scale when the outcome has been log-transformed (to fit the model) as we see below.

After taking a log transformation of $Y$, the regression model is now $\log Y_i=\beta_0+\beta_1 x_i+\varepsilon_i$, where $\varepsilon_i \sim N(0,\sigma^2)$ and therefore $\text{E}[\log (Y)]=\beta_0+\beta_1 x$. We would then fit the model and check our model assumptions and hopefully the log transformation of $Y$ has ensured that the residuals are normally distributed etc. Now, how do we interpret the results?

Exponentiating both sides of the above equation (to get back to the original scale), and recalling that $GM(Y)=e^{\text{E}[\log (Y)]}$, we can obtain an interpretation of the parameter $\beta_1$ in terms of the **geometric mean** of $Y$. i.e., $e^{\text{E}[\log (Y)]}=e^{\beta_0+\beta_1 x} = GM(Y)$.

Thus, if we were to interpret the linear regression results on the *original scale* (back-transformed), we would need to interpret the exponentiated regression parameters (e.g. $e^{\beta_1}$) in terms of their effect on the **geometric mean** of $Y$.

Introducing the notation $GM(Y|x)$ to denote the geometric mean of $Y$ at a given value of the covariate $x$, we now examine the effect on $GM(Y|x)$ for a change of 1 unit in $x$: $$\frac{\mbox{GM}(Y \mid x+1)}{\mbox{GM}(Y \mid x)}
  =\frac{e^{\text{E}[\log Y \mid x+1]}} {e^{\text{E}[\log Y \mid x]}}
  =\frac{e^{\beta_0+\beta_1(x+1)}} {e^{\beta_0+\beta_1x}}
  =\frac{e^{\beta_0} e^{\beta_1x} e^{\beta_1}} {e^{\beta_0} e^{\beta_1x}}
  =e^{\beta_1}.$$ Therefore increasing $x$ by 1 unit is associated with a *proportionate* (multiplicative) increase of $e^{\beta_1}$ in the geometric mean of $Y$.

Example interpretation based on the above: For every 1 unit increase in $x$ there is a multiplicative increase in the geometric mean of $Y$ of $e^{\beta_1}$.
...
Almost all the data are bunched up in the bottom left hand corner (left plot). However, a logarithmic transformation of both brain and body weight (right plot) reins in the extreme values on both axes and based on the scatterplot is worth pursuing with linear regression.

The regression model that would be fitted is $\log Y_i=\beta_0+\beta_1\log x_i+\varepsilon_i$. The parameter $\beta_1$ reflects the effect on $\text{E}[\log Y$\] of [additive]{.underline} changes in $\log(x)$, which corresponds to [multiplicative]{.underline} changes in $x$. Therefore let us examine the proportionate change in $GM(Y)$ for a *doubling* (i.e. multiplying by a factor of 2) of covariate $x$.
...
The estimated slope for this regression is 0.752 (regression output not shown here), and so $2^{0.752}$ = 1.68. Therefore, we estimate the geometric mean brain weight to increase 1.68-fold or by 68% for each doubling in weight of mammals. The 95% confidence interval for $2^{\beta_1}$ is obtained from the 95% confidence interval for $\beta_1$ (say (L,U) ) by raising 2 to the power of the endpoints, namely ($2^{L}, 2^{U}$), producing (1.62, 1.75) for this example.

This example is a rather extreme case with respect to the distribution of the covariate and it is probably fair to say that transformations of $x$ are not usually as important to consider as transformations of the outcome. Since the linear model takes the values of $x$ as given, so there is no modelling of their distribution, there is no formal need to obtain a "nice" symmetric distribution of the independent variable/covariate. However, when we discuss the concept of leverage in Module 3 we will see that highly skewed $x$ distributions are undesirable because they may lead to just one or two observations having an unduly large influence on the estimated regression model.

Note that although we have focused on log-transformations here, other transformations are possible, such as square root, square (or other powers), logit - some of which we will use later in this unit.

### Optional Reading - Book Chapter 4: Section 4.7.5-4.7.6 (pages 128-130). {#reading_wk02_transfo .unnumbered}

You may wish to read p128-130 of the textbook for alternative explanations of how to interpret log-transformed data (in terms of percentage increases/decreases).
...
Note that this method is outside the scope of RM1 - we just mention it here for your interest.

## Exercise {.unnumbered}

[assumptions.csv](https://raw.githubusercontent.com/atpinto/RM1/refs/heads/main/Data/assumptions.csv){target="_blank"} consists of simulated data for you to practice checking the assumptions of simple linear regression. It contains 4 continuous outcome variables `y1`, `y2`, `y3` and `y4`. It has two explanatory variables `x1` and `x2`. Your task is to investigate the assumptions of any or all of the possible different simple linear regressions between the outcome variables and the explanatory variables. There are 8 possible regressions to check in total (4 choices for outcome variable, and 2 choices for explanatory variable. $4\times2=8$).

Discuss your answers in the discussion board. 
