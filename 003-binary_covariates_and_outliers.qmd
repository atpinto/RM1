...
source("setup.R")
```
# Binary Covariates in Linear Regression, Outliers, and Influential Observations {#binary_outliers}
...
A **leverage point** is an observation with an unusually high or low value for a predictor variable (i.e. lies far away from the other values for that predictor variable). Such points may be considered "outliers" (using the lay meaning of "outlier", rather than our definition above) in terms of the predictor values because they fall well outside the typical range of the other observations for that variable (unusual in the x-direction). These points also have the potential to affect the regression line.
:::
::: g-col-5
...
A data point may not have a large residual but still have an important influence in the estimated regression line. Below, you can see that the data point on the left does not appear to have a large residual but it strongly affects the regression line. 
...
Leverage is a measure of how far away each value of the independent variable is from the others. Data points with high-leverage, if any, lie some distance away from the other values for the independent variables. The leverage ($L$) values range between 0 and 1.
...
We can also compute how the coefficients change if each observation is removed from the dataset. This will produce a vector for $\beta_0$ and $\beta_1$ corresponding to $n$ regressions fitted by deleting each observation one-at-a-time. The difference between the full dataset estimates and the estimates obtained by removing each data point is called *DFBETA*. In the example of the small simulated dataset above, the dfbeta can also be obtained from the `influence()` function in R.
...
**Stata code**
We are not aware of a function in Stata to compute the unstandardised version of DFBETA (mentioned above); you would have to write your own code to do this - see below.

```{stata, collectcode=TRUE, collapse=TRUE }
*We are not aware of a function in Stata to compute 
*the unstandardised version of DFBETA.
*This script computes the unstandardised DFBETA "manually"
...
Note that the *DFBETA* above are computed in the *original scale* of the data. Thus, the magnitude of the difference is dependent on this scale. An alternative is to standardise the *DFBETA* by dividing them by the standard error of the corresponding coefficient estimate. This expresses the influence of an observation as the number of standard errors by which the coefficient changes when that observation is excluded. Large *DFBETA* values indicate that an observation has a significant impact on the estimated regression coefficient. A common cut-off for a very strong influence in the results is the value $2$.
We are not aware of a function in Stata to compute the unstandardised version of DFBETA (mentioned above); you would have to write your own code to do this - see below.
...
The code below is for computing the standardised DFBETA in R.
...
The code below is for computing the standardised DFBETA in Stata.
...
::::: grid
::: g-col-8
This is another measure of influence that combines the leverage of the data point and its residual. The **Cook's distance ($D_j$)** summarises how much all the values in the regression model change when the $j-th$ observation is removed. 
:::
...
:::
:::::
A general rule of thumb is that a point with  $D_j > 4/n$ is considered to be an influential point.
...
The above measures are commonly represented in a graphical way. There are many variations of these plots. Below are some examples of these plots but many other plots are available in different packages.
...
This reading supplements the notes above with emphasis on the DFBETA plots. Note that this sub chapter appears in the book after the extension of simple linear regression to the use of multiple independent variables (covariates) in the regression model, which we have not yet covered (Module 4). However, there are only a few references to the multiple linear regression case.

## Exercises: {.unnumbered}

The dataset [lowbwt.csv](https://raw.githubusercontent.com/atpinto/RM1/bdb2b60a471ce0d99c359c4b5283eb2cf6cb2883/Data/lowbwt.csv) was part of a study aiming to identify risk factors associated with giving birth to a low birth weight baby (weighing less than 2500 grams).
...
```{r, include=F, eval=F}
lowbwt <- read.csv("https://raw.githubusercontent.com/atpinto/RM1/bdb2b60a471ce0d99c359c4b5283eb2cf6cb2883/Data/lowbwt.csv")   
...
1. Linear regression with a binary covariate is equivalent to a t-test (that assumes equal variance).

2.  **Outliers** are observations with a very large absolute residual value. That is, we normally refer to "outliers" as observations with extreme values in the outcome variable $Y$ (i.e. are "unusual" in the $Y$-axis).

3. **Leverage points** are observations with unusual covariate $x$ values (have high "leverage"). The precise formula for leverage is less important than understanding how high leverage observations can impact your regression.

4.  A residual versus leverage plot is a very useful diagnostic to see which observations may be highly influential
...
5.  When potentially highly influential observations are detected, a sensitivity analysis where the results are compared with and without those observations is a useful tool for measuring influence.
:::
