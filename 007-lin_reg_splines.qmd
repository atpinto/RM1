...
1.  Have a general understanding of the use of flexible techniques, such as polynomial regression, restricted cubic splines, and fractional polynomials, that can be used to model complex, non-linear relationships.

2.  Be able to run polynomial regression models

3.  Be able to incorporate restricted cubic splines into linear regression models
...
| Reading               | 4                   |
| Lecture 2             | 4, 5                |
| Independent Exercises | 2, 4                |

## Introduction to linear splines {.unnumbered}

::::: grid
::: g-col-6
**Lecture 1 in R**

<a href="https://www.youtube.com/embed/b1Vm9Mg3GyM" onclick="window.open(this.href, 'videoPopup', 'width=800,height=600'); return false;"> <img src="https://img.youtube.com/vi/b1Vm9Mg3GyM/hqdefault.jpg" width="200"/> </a>
:::

::: g-col-6
**Lecture 1 in Stata**

<a href="https://www.youtube.com/embed/oSib7b4FtP0" onclick="window.open(this.href, 'videoPopup', 'width=800,height=600'); return false;"> <img src="https://img.youtube.com/vi/oSib7b4FtP0/hqdefault.jpg" width="200"/> </a>
:::
:::::
...
To motivate, we consider data on 892 females under 50 years collected in three villages in West Africa. Investigators are interested in exploring the relationship between age and triceps skinfold thickness, a crude measure of body fat. The dataset is called *triceps* and include:

-   *age* - age in years.

-   *thick* -  triceps skinfold thickness in mm
    
    
-   *logthick* -  logarithm of triceps skinfold thickness.

The figure below displays the the relationship between *age* and *logthick*. It is immediately clear that this relationship is nonlinear across such a wide age span. 

A nonparametric smoother (LOWESS) can be applied to the data and returns the blue fitted line (see plot below) confirming the visual impression.
...
A simple regression model of the form $logthick_i=\beta_0+\beta_1 age_i+\epsilon_i$ for $i=1,\dots,n=892$ would hardly give an approximation of the observed pattern, so the question arises: how can we modify the regression model to make it more realistic? 

The first strategy is to replace the linear function of age by a *cubic polynomial*: $f(age)=\beta_0+\beta_1 age +\beta_2 age^2 + \beta_2 age^3$. 
The term "cubic" is used because we are fitting a polynomial of degree 3 due to the largest exponent being 3. A quadratic curve could also be fitted but would not be appropriate given the shape of the plot. Such a model is naturally called *polynomial regression*, irrespective of the degree of the polynomial function $f(x)$. Although we can define the quadratic and cubic terms by hand, it's preferable to use the built-in tool *poly(x,3)* in R to define the polynomial (here $x$ refers to the variable of interest, e.g. $x=age$). This leads to a simpler univariate smooth model of the form: $y_i=f(x_i)+\epsilon_i$ where $y$ is the generic term for the response ($y=logthick$ in our example).
...
Although all the polynomial coefficients are significant (results not shown), we should *not* use significance (i.e. the p-value of a particular parameter) to decide whether to include a particular term or not. Another parameterisation that is based on *orthogonal* polynomials is often preferable as it is more stable, especially for high-degree polynomials. Using this (orthogonal) parameterisation, which is the default parameterisation in R (*poly(age, 3, raw="FALSE")* or *poly(age, 3)*), yields a non-significant quadratic term. The quadratic term is nevertheless needed to give the same fit. The *F-test* (or a *likelihood ratio test (LRT)*) can help decide whether the cubic polynomial in age is necessary and is, for instance, better than a quadratic polynomial or a simple linear fit. We leave this as an exercise for you to complete (e.g., using *anova(model1, model2)* in R, or the *lrtest model1 model2* or *test age2 age3* command in Stata; or some variations of these).

::: {.callout-tip}
Use F-tests or likelihood ratio tests (LRT) to compare models to see whether higher order terms are needed (e.g. to compare a cubic polynomial model to a quadratic polynomial model, or a simple linear model). Do not just rely on the p-value for each term/parameter in the model.
...
The disadvantage of polynomials is that they tend to fit the data globally and are not very good locally, for instance around year 10 for *logthick* in the plots above. Here we introduce another type of transformation of an $x$ variable to help model non-linear relationships between $x$ and $Y$.

One way to get around this issue is to partition the range of $age$ (or the covariate $x$ in general) into smaller intervals, utilising a small number of points called *knots*. We can then fit (cubic) polynomials locally but by doing this we will get discontinuities. A simple remedy is to impose constraints on these polynomials so that the global result is a *smooth* function. We typically do this by forcing the successive derivatives of the function to be continuous up to some order (e.g. 2). The resulting function is called a *spline*. 

Different splines exist and a common type consists of portions of cubic polynomials in the middle, joined together at the knots that become linear at the edges (i.e before the first knot and after the last one). Such splines are called *Restricted Cubic Splines (RCS)* with RCS(4) denoting an RCS with 4 knots and RCS(5) denoting an RCS with 5 knots. Restrictions are imposed to the different parts to yield a smooth function. It is not necessary to know their complex algebraic expression but we can see how they look in Figure 7.1 below where we fit an RCS(5) model:
...
There are 4 terms because we chose to use 5 knots (i.e, d.f. = number of knots minus 1) with $S_1(age)=age$ in blue, $S_2(age)$ in red, $S_3(age)$ in magenta, and $S_4(age)$ in green on the left panel. **Each individual spline function (called spline basis) should not be interpreted individually.** What matters is their "combined effect", i.e the function $$f(age)=b_0 + b_1S_1(age)+b_2S_2(age)+b_3S_3(age)+b_4S_4(age) $$ which is very smooth and seems to capture the features of the data well. Here the coefficients $b_0,b_1,\dots,b_4$ were calculated by fitting $f(a)$ to the data. Such a fit is possible using standard commands since the function is *linear in the coefficients* despite its *nonlinearity in age*.
...
The command *rcs(age,4)* specifies an RCS in age with 4 knots placed at their default location. Note that the first two lines after calling the *rms* library are not absolutely needed to get the fit but they will be used to plot the splines, so it's recommended to add them anyway. The output is given below:
```{r, message=FALSE, collapse=TRUE}
fit.rcs
```

You may notice that the syntax is slightly different from the standard way to display an output after typing the *lm* command since *summary()* is not used. The display is rather unusual and indicates the coefficients for the various spline bases with the notation $age^\prime$ and $age^{\prime\prime}$ corresponding to two additional splines terms $S_2(a)$ and $S_3(a)$ that are never interpreted separately.

**Stata code and output**

The Stata syntax still uses the usual *regress* command but it's preceeded by a command *mkspline* explaining what splines need to be fitted. Below is the syntax and output in Stata.
...
The command *mkspline age_spl = age, cubic nknots(4)* asks Stata to create an RCS in age with 4 knots; Stata will generate the corresponding terms called *age_spl1*, *age_spl2* and *age_spl3*. Note that *age_spl1* is always *age* (i.e., the linear term for age) so that the syntax is equivalent to *regress logthick age age_spl2 age_spl3*. The generic command with \* makes sure that *all* age terms are included. Some other options are available that you can explore using the Stata help file.


What remains to be discussed is: 

1.    How do we choose the knots?

2.    How many knots do we choose?

3.    How do we know that the splines are needed?


### Choosing the knot locations and their number {.unnumbered}

Typically a **small** number of knots is recommended, say between 3 and 5. We chose 5 knots earlier because we have a lot of data in our example dataset, but 4 is often appropriate. Their location is critical and up to the investigator. A default choice has  fortunately been implemented in Stata and R at specific quantiles of the variable of interest, and these typically produce good model fits. The two figures below gives you the data and 2 different fits, one with 4 knots located using the default knot locations (Figure 7.2) and at age 10, 20, 35, 45 (Figure 7.3). We have added the vertical dotted lines to indicate their location (this is done for educational purposes here and wouldn't often be done in practice).
...
**Figure 7.2:** An RCS fit with 4 knots (default location)
...
```{r, collectcode=TRUE, collapse=TRUE }
# Testing whether the additional splines terms are necessary
anova(fit.rcs4a) #model for figure 7.2 using the default locations for knots
# look at age nonlinear with 2 df. Highly significant. Splines necessary
```

We see that the "nonlinear" terms for age (with 2 df) are significant (p<0.0001), which means that the splines are necessary (when we fit the model using the default knot locations). 



```{r, collapse = TRUE, results="hide"}
# Figure 7.3 - 4 knots at age 10, 20, 35 and 45
fit.rcs4b <- ols(logthick ~ rcs(age,c(10,20,35,45)),data=triceps)
pred4b<-predict(fit.rcs4b)
plot(logthick~age, xlab="Age",ylab="Skinfold thickness (log)",data=triceps)
lines(triceps$age,pred4b,  col="blue", lwd=2,lty=2)
abline(v=c(10,20,35,45),lty=2,col="blue")
# poor fit for lower age; to produce a figure without fancy colours, do not run the last 3 lines
mini<-max(which(triceps$age <10))
maxi<-min(which(triceps$age >=45))-1
lines(triceps$age[mini:maxi],pred4b[mini:maxi],  col="red", lwd=2,lty=1)
...
We see that the "nonlinear" terms for age (with 2 df) are significant (p<0.0001), which means that the splines are necessary (when we fit the model using 4 knots at age 10, 20, 35 and 45). 
...
### Comparing the models fitted so far {.unnumbered}

Now we have produced 3 different fits (models) of the same data (ignoring the RCS(5) fit that was given to show what a spline function is):

1. Cubic polynomial
2. RCS(4), default knots
3. RCS(4), knots at age 10, 20 35, 45. 

Visually, it's clear that 2. (RCS(4), default knots) gives the better fit, so this is the one we'll choose. There are many other fits/models you could get by playing with the number of knots and their location, so you could add them to the list. There is a way to compare such models that are not nested, but we defer the explanation to week 8 (and hopefully the approach in week 8 agrees with the visual impression).
...
### Investigation/Exercises {.unnumbered}

The objective of this activity is to become familiar with RCS (and polynomials) using the *hers* data

1)  Start by reading p. 109 - 114 of Vittinghof et al. (2012) where the authors explore modelling *HDL* as a nonlinear function of *BMI*, adjusted for a few covariates, namely *age*, *nonwhite*, *smoking* and *drinkany*. It is suspected that the association with BMI may not be linear. Begin by fitting a quadratic model for BMI, adjusting for these additional covariates. See Table 4.20, p. 111 for more details. (Note that the authors performed some centring of the BMI and age variables and used this in the model p110/p111).

2)  Fit an RCS in BMI with 4 knots (default locations) while adjusting for the other covariates. Does it change the conclusion regarding the linearity of BMI? Test whether the additional spline terms in BMI are necessary for this data. Note that if you want to reproduce the results of Table 4.21 p. 114, you need to use *age10* (instead of *age* or *agec*) and have 5 knots (not requested here for the exercises).

3)  Plot the fitted line with its 95% CI band. To get a nice plot you need to fix the *other* covariates than BMI. One way to do this is to set these covariates at the mean or median value. 

::: {.callout-note}
R users: The latter is provided by the R command *plot(Predict(fit, BMI))* where *fit* is the RCS fit obtained earlier (Question 2). Stick to the plots obtained after fitting an RCS using *rms* and *ols* (and avoid using *lm*).
...
Note that we will practice this in the Week 7 Tutorial.
:::

4)  Change the location of the 4 knots (your choice) and refit the model. Try also a model with a different number of knots. Does it change your conclusion? Provide a qualitative interpretation of the BMI effect in the model you decide to keep.

5)  Do we need an RCS model for age?

## Fractional polynomials and other methods {.unnumbered}

RCS are not the only approach we could use to model nonlinearities. Other types of splines exist including smoothing splines that impose a penalty for a lack of smoothness; they are more nonparametric by nature. 

Here we would like to say a few words about *fractional polynomials* that were introduced by Royston and Atlman (1994) and have been further developed since then. For the sake of simplicity, we describe the procedure using a model with a single continuous covariate (called $x$). One way to generalise the linear combination $\beta_0 +\beta_1 x$ is to replace it by a special type of polynomial that might include logarithms, noninteger powers, and repeated powers. One can show that, with a suitable range of powers, such polynomials provide a considerable range of functional forms in $x$ that are useful to model real data. They are called "Fractional Polynomials" (FP) to distinguish from standard polynomials. 

The default set of powers from which FP powers are selected is typically $\{−2, −1, −0.5, 0, 0.5, 1, 2, 3\}$, with 0 signifying $\log$. There is also the convention that, every time a power repeats in an FP function of $x$, it is multiplied by another $\log(x)$. To describe the FP we typically list the series of powers that will be used.

For example, a FP in $x$ with powers $(−1, 0, 0.5, 1)$ and coefficients $\beta$ has the following form: $$\beta_1 x^{-1}+\beta_2 \log(x) + \beta_3 x^{1/2} + \beta_4 x$$ 

Also, a FP with powers $(0, 1, 2, 2)$ will correspond to $$\beta_1 \log(x) + \beta_2 x +\beta_3 x^2 +\beta_4 x^2\log(x)$$

Now the key issue will be to choose the best powers. Some automated procedures are available but we strongly suggest that you keep things simple and use plots (possibly some obtained using splines) to guide your choice. We do not present here a full account of this approach (available in Stata through the commands *fp* and *mfp* and in R via the library *mfp*); it's more a note mentioning that they exist and are part of a suite of techniques that could be used in model building. More will be said in week 11 in the context of logistic regression where we have learned more tools to decide on how to compare different models. 

Finally, splines and FP are not necessarily opposed. There will be situations like the triceps data where splines are more appropriate due the local flexibiliy they provide, others where FP can be valuably used to capture more general trends that can be extrapolated, and situations where they complement each other.


You can use the interactive simulation below using an hypothetical example of a non-linear association between systolic blood pressure and body mass index, and see how different models fit a complete non-linear effect. 

<iframe src="https://atpinto.github.io/MultipleLinearRegression/nonlineareffects.html" width="100%" height="860" title = "Fitting nonlinear effects">
</iframe>
...
<a href="https://www.youtube.com/embed/68fndnP_PV0" onclick="window.open(this.href, 'videoPopup', 'width=800,height=600'); return false;"> <img src="https://img.youtube.com/vi/68fndnP_PV0/hqdefault.jpg" width="200"/> </a>
:::

::: g-col-6
**Lecture 2 in Stata**

<a href="https://www.youtube.com/embed/2PWYd8fWniw" onclick="window.open(this.href, 'videoPopup', 'width=800,height=600'); return false;"> <img src="https://img.youtube.com/vi/2PWYd8fWniw/hqdefault.jpg" width="200"/> </a>
:::
:::::
...
# histogram of the replicates + normal probability plot
...
To understand better how the bootstrap works in linear regression, we ask you to carry out the following tasks:
...
4)  Fortunately, both Stata and R have a built-in command that avoids having to do the resampling "by hand". Use Part B) of the code to provide the 3 types of bootstrap 95% CI. Do you see any meaningful difference with the standard least squares (LS) analysis?
...
As discussed over the previous weeks, a critical assumption for the normal theory or even the bootstrap methods to be valid is the assumption of constant variance of the error term (homoscedasticity). What can we do if this assumption is not met? 

A standard way to deal with this issue (known as heteroscedasticity) is to transform the endpoint, but such a variance-stabilising transformation does not always exist. 

Another way is to use weighted LS that provide valid inference in situations where the variance of the error term is $var(\varepsilon)=\sigma^2 W^{-1}$, the only issue with this approach is to find a proper symmetric matrix, $W$, often chosen to be a diagonal matrix. Its choice can be driven by plots, for instance a plot of residuals vs covariate, but it's not always straightforward. 

Another convenient means of dealing with nonconstant residual variance is to use the so-called "robust" covariance matrix (also known as the "White covariance matrix" or the "sandwich estimator"). The corresponding SEs allows reliable inference when the constant-variance assumption is violated. This little technical note explains how this can be done: 

1. Robust SEs can be calculated in Stata using the option *vce(robust)* of the *regress* command with a more accurate version *vce(hc3)* being preferable in small samples. 

2. R users can install the libraries *sandwich* and *lmtest* and then use *vcovHC*. The same options and a few other alternatives can be obtained through a command like this: *coeftest(m, vcov. = vcovHC(m, type = 'HC3'))* where *type = "HC3"* specifies the chosen option, here the same as Stata's *vce(hc3)*. The command to match Stata's output for the 95% CI is: *coefci(out, vcov. = vcovHC(out, type = 'HC3'))*. Of course, the object *out* contains the result of the initial *lm* fit. 

You can explore these options, for instance, you can reproduce the fits given p. 123 of Vittinghof et al. (2012) using the *hers* dataset and the appropriate sandwich matrix. A textbook example is given in the second lecture so you can also explore that example.
...
4.  Robust variance-covariance matrices / Sandwich formulas exist to cope with heteroscedasticity issues (e.g. non constant variance) when no variance-stabilising transformation exists.
